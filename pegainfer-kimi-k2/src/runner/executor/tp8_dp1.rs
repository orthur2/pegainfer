...
impl ForwardExecutor for Tp8Dp1ForwardExecutor {
    ...
    fn forward_prompt_len1_batch(
        ...
        if token_ids.is_empty() {
            bail!("Kimi prompt_len1 batch requires at least one token");
        }
        if token_ids.len() != slots.len() {
            bail!(
                "Kimi prompt_len1 batch input mismatch: tokens={}, slots={}",
                token_ids.len(),
                slots.len()
            );
        }
        let responses = self
            ...
            .iter()
            .map(|worker| {
                worker.forward_prompt_len1_batch_next_tokens_async(
                    token_ids.to_vec(),
                    slots.to_vec(),
                    decode_batch_size,
                )
        ...
        for response in responses {
            rank_reports.push(response.recv().map_err(|_| {
                anyhow::anyhow!("Kimi-K2 rank worker dropped prompt_len1 batch response")
            })??);
        }
        for (rank_idx, reports) in rank_reports.iter().enumerate() {
            ensure!(
                reports.len() == token_ids.len(),
                "Kimi rank {rank_idx} returned {} prompt_len1 reports for {} rows",
                reports.len(),
                token_ids.len()
            );
            for (row, report) in reports.iter().enumerate() {
                validate_one_token_report(report, "prompt_len1", slots[row], token_ids[row])?;
            }
        }
        ...
        for row in 0..token_ids.len() {
            let best = rank_reports
                ...
                .ok_or_else(|| {
                    anyhow::anyhow!("Kimi runtime produced no prompt_len1 report for row {row}")
                })?;
            selected.push(best);
        }
        Ok(selected)
