...
//! Default-off, this crate exposes only `HW_CUDA_ENABLED` / `HW_RDMA_ENABLED`
//! diagnostic markers.

/// Whether the `hw-rdma` feature is active. Diagnostic only. (When on, this
/// crate transitively activates `hw-cuda` on `cuda-lib` / `torch-lib` /
/// `a2a-kernels`, since the all-to-all path needs both subsystems.)
