...
//! comparison the gate bounds a two-sided |Δlogprob| distribution: openinfer's
...
//!     `prompt + tail[..i]` with max_tokens=1. openinfer's pick must satisfy
...
/// Per-position regret rule: openinfer's pick must satisfy
///
...
/// committed vLLM fixture, so openinfer cannot influence its own
/// tolerance.
///
...
/// Aggregate guard: per pass, the fraction of positions where openinfer's
...
/// engines pick the same token, Δ between openinfer's exact host log-softmax
...
fn model_path() -> String {
    let path = std::env::var("OPENINFER_TEST_MODEL_PATH").unwrap_or_else(|_| {
        panic!(
            "kimi vllm_golden_gate: OPENINFER_TEST_MODEL_PATH is not set. \
...
/// Fold openinfer's pick at one position into the stats, applying the
...
fn check_pick(stats: &mut RegretStats, seq: &Seq, pos: usize, pick: u32, pick_lp: &TokenLogprob) {
    ...
    if pick_lp.logprob < own_top - 1e-5 {
        stats.violations.push(format!(
            "{} pos {pos}: pick {pick} logprob {:.4} sits below openinfer's \
    ...
    match seq.vllm_logprob(pos, pick) {
        None => stats.violations.push(format!(
            "{} pos {pos}: openinfer picked {pick}, absent from vLLM's top-{} \
        ...
        Some(lp) => {
            ...
            if regret <= bound {
                ...
                stats.violations.push(format!(
                    "{} pos {pos}: openinfer picked {pick}, which vLLM scores \
